...
# serial line
if config.DEVICE == "pico":
    import serial

    _serial = serial.Serial(config.PICO_PORT, baudrate=460800, timeout=1)
...
def _update_pico():
    ...
    for i in range(pixels.shape[1]):
        frame.append(p[0][i])  # R
        frame.append(p[1][i])  # G
        frame.append(p[2][i])  # B

    frame = bytes(frame)
    n = len(frame) // 3
    packet = bytes([0xFF, 0x00, n]) + frame
